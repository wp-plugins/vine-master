<?php
if( is_multisite() ) {
	function menu_multi_vine_admin_shortcodes(){
	// Create menu
	add_submenu_page( 'vine-master', 'Shortcodes', 'Shortcodes', 'manage_options', 'vine-master-admin-shortcodes', 'vine_master_admin_shortcodes' );
	}
}
else {
	// Create menu
	function menu_single_vine_admin_shortcodes(){
		if ( is_admin() )
		add_submenu_page( 'vine-master', 'Shortcodes', 'Shortcodes', 'manage_options', 'vine-master-admin-shortcodes', 'vine_master_admin_shortcodes' );
	}
}

function vine_master_admin_shortcodes(){
?>
<div class="wrap">
...
<?php
if(!class_exists('vine_master_admin_shortcodes_table_in')){
	require_once( dirname( __FILE__ ) . '/vine-master-admin-shortcodes-table-in.php');
}
//Prepare Table of elements
$wp_list_table = new vine_master_admin_shortcodes_table_in();
//Table of elements
...
<?php
if(!class_exists('vine_master_admin_shortcodes_table_un')){
	require_once( dirname( __FILE__ ) . '/vine-master-admin-shortcodes-table-un.php');
}
//Prepare Table of elements
$wp_list_table = new vine_master_admin_shortcodes_table_un();
//Table of elements
$wp_list_table->display();
?>
</br>
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_vine_admin_shortcodes' );
}
else {
add_action( 'admin_menu', 'menu_single_vine_admin_shortcodes' );
}
?>