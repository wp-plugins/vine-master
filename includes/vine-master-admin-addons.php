<?php
if( is_multisite() ) {
	function menu_multi_vine_admin_addons(){
	// Create menu
	add_submenu_page( 'vine-master', 'Add-ons', 'Add-ons', 'manage_options', 'vine-master-admin-addons', 'vine_master_admin_addons' );
	}
}
else {
	// Create menu
	function menu_single_vine_admin_addons(){
		if ( is_admin() )
		add_submenu_page( 'vine-master', 'Add-ons', 'Add-ons', 'manage_options', 'vine-master-admin-addons', 'vine_master_admin_addons' );
	}
}

function vine_master_admin_addons(){
?>
<div class="wrap">
...
<h2><b>&nbsp;<?php echo get_option('vine_master_name'); ?> Add-ons</b></h2>
<?php
if(!class_exists('vine_master_admin_addons_table')){
	require_once( dirname( __FILE__ ) . '/vine-master-admin-addons-table.php');
}
//Prepare Table of elements
$wp_list_table = new vine_master_admin_addons_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_vine_admin_addons' );
}
else {
add_action( 'admin_menu', 'menu_single_vine_admin_addons' );
}